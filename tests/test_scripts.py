...
def test_install():
    ...
    assert "pip install madoop" in install_content
...
def test_servers_start(setup_teardown):
    ...
    db_path = Path("var/index.sqlite3")
    if db_path.exists():
        db_path.unlink()
    ...
    shutil.copy(utils.TESTDATA_DIR/"index.sqlite3", db_path)
...
def test_indexdb(setup_teardown, tmpdir):
    ...
    # Create tmp directory containing search_server/search/sql/index.sql
    Path(tmpdir/"search_server/search/sql").mkdir(parents=True)
    shutil.copy(
        utils.TESTDATA_DIR/"small.sql",
        tmpdir/"search_server/search/sql/index.sql",
    )

    # Run indexdb create and verify var/index.sqlite3 was created
    ...
    db_path = tmpdir/"var/index.sqlite3"
    assert db_path.exists()

    # Try to create the database again, should get an error
    completed_process = subprocess.run([indexdb_path, "create"])
    assert completed_process.returncode != 0

    # Run indexdb destroy and verify var/index.sqlite3 deleted
    subprocess.run(
        [indexdb_path, "destroy"],
        cwd=tmpdir,
        check=True,
    )
    assert not db_path.exists()

    # Run indexdb reset and verify var/index.sqlite3 created
    subprocess.run(
        [indexdb_path, "reset"],
        cwd=tmpdir,
        check=True,
